...
- Interactive TUI container browser ([tview](https://github.com/rivo/tview)) with keyboard navigation and in-place actions
- Interactive image pull dialog with tag selection
- Import container backups as Docker images via Docker Go SDK
- Color terminal output via [Charm](https://charm.land) (lipgloss)
...
COMMANDS:
  ...
  pull [--image <image>]              Pull/update container image (interactive dialog if no --image)
  import <file> --repo <repo>         Import a backup tar archive as a Docker image
  list                                List all SCRT containers (interactive TUI in TTY)
...
EXAMPLES:
  ...
  scrt import backup.tar --repo fonalex45/scrt --tag restored
  scrt list                           # interactive TUI with e/s/d/b/r/q keys
  scrt list | cat                     # static table (non-TTY / piped output)
  scrt config edit
```
...
│   ├── container/      # Docker SDK + CLI operations (lifecycle, builder, labels)
│   ├── project/        # Project directory scaffolding and backup
│   └── tui/            # tview interactive container browser and pull dialog
...
SCRT is written in Go using the [Docker Go SDK](https://pkg.go.dev/github.com/docker/docker/client) for non-interactive operations and `os/exec` only for TTY-attached sessions (`docker run -it`, `docker exec -it`). CLI parsing is handled by Cobra. The codebase follows these principles:
...
- **Terminal UI**: [tview](https://github.com/rivo/tview) for interactive container browsing; [lipgloss](https://github.com/charmbracelet/lipgloss) for non-TUI styling; static table fallback for piped/non-TTY output
...
| Docker Go SDK migration (non-interactive ops) | ✅ Complete |
| tview interactive container browser (`scrt list`) | ✅ Complete |
| Interactive image pull dialog (`scrt pull`) | ✅ Complete |
| `scrt import` — import backup tar as Docker image | ✅ Complete |
| Charm TUI — color output, config display | ✅ Complete |
...
| Release pipeline decoupled from release-please (tag-triggered + `workflow_dispatch`) | ✅ Complete |

v3.0.0 will be cut with a `feat!:` commit. The major version bump reflects the architectural upgrade to the Docker Go SDK and tview TUI.
...
Merging a `feat:` or `fix:` commit to `main` causes release-please to open a release PR. Merging that PR creates the version tag. Pushing **any** `v*.*.*` tag triggers the full release pipeline — tests, linux/amd64 binary upload, and Docker image push. The pipeline can also be triggered manually from the **Actions → Release → Run workflow** UI when a release needs to be re-run or was created outside of release-please.

> [!NOTE]
> Avoid Go-style function calls (e.g. `foo(bar)`) in commit message bodies — the release-please parser treats `(` as a scope delimiter and will silently skip the commit, preventing a release PR from being opened.
